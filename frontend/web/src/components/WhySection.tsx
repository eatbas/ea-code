...
function ComparisonRow({ single, ea }: ComparisonRowProps) {
  return (
    <div className="grid grid-cols-2 gap-4 border-b border-border py-4 last:border-b-0">
      <div className="flex items-start gap-3">
        <span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-red-500/15 text-red-400">
          <X className="h-3 w-3" />
        </span>
        <span className="text-sm leading-relaxed text-muted">{single}</span>
      </div>
      <div className="flex items-start gap-3">
...
const COMPARISONS: ComparisonRowProps[] = [
  ...
  },
  {
    ...
    ea: "Auto-loops with full context until the judge approves",
  },
  {
    single: "Context gets lost between sessions",
    ea: "Session memory carries continuity across runs",
  },
  {
  ...
  return (
    <section id="why" className="relative py-24 md:py-32">
      <div className="mx-auto max-w-4xl px-6">
        ...
        <div className="fade-in-up rounded-2xl border border-border bg-surface-elevated p-6 md:p-8">
          {/* Header */}
          <div className="mb-2 grid grid-cols-2 gap-4 border-b border-border pb-4">
