...
export function ProjectThreadsList({
  ...
  const [confirmingProjectPath, setConfirmingProjectPath] = useState<string | null>(null);
  ...
  function handleProjectRemoveClick(e: React.MouseEvent, projectPath: string): void {
    e.stopPropagation();
    setConfirmingProjectPath(projectPath);
  }

  function handleProjectConfirm(e: React.MouseEvent, projectPath: string): void {
    e.stopPropagation();
    onRemoveProject?.(projectPath);
    setConfirmingProjectPath(null);
  }

  function handleProjectCancel(e: React.MouseEvent): void {
    e.stopPropagation();
    setConfirmingProjectPath(null);
  }
      ...
      <div className="px-2">
        ...
        <div className="flex flex-col gap-1 pb-3">
          {projects.map((project) => {
            ...
            const isConfirmingProject = confirmingProjectPath === project.path;

            return (
              <div key={project.path} className="rounded-lg">
                {isConfirmingProject ? (
                  <div className="flex items-center gap-1 rounded-lg bg-[#24243a] px-3 py-2">
                    <span className="flex-1 truncate text-xs text-[#e4e4ed]">Remove project?</span>
                    <button
                      onClick={(e) => handleProjectConfirm(e, project.path)}
                  ...
                  <div
                    className={`group/project flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm transition-colors ${
                      isActiveProject
                        ? "text-[#e4e4ed]"
                  ...
                  >
                    ...
                    {onRemoveProject && (
                      <button
                        onClick={(e) => handleProjectRemoveClick(e, project.path)}
                        className="hidden shrink-0 rounded p-0.5 text-[#6f7086] hover:text-[#ef4444] group-hover/project:block transition-all"
