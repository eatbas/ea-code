...
export function PrerequisiteBanner({ status, onDismiss }: PrerequisiteBannerProps): ReactNode {
  ...
  return (
    <div className="fixed top-0 right-0 left-0 z-50 border-b border-[#ef4444]/30 bg-[#1a1a24]/95 px-4 py-3 shadow-lg backdrop-blur-sm">
      <div className="mx-auto flex max-w-3xl items-start gap-3">
        ...
        <div className="flex-1 space-y-1">
          <p className="text-sm font-medium text-[#e4e4ed]">Missing prerequisites</p>
          {missing.map((item) => (
            <p key={item.label} className="text-xs text-[#9898b0]">
              {item.label}{" "}
              <a
                href={item.url}
                target="_blank"
        ...
        <button
          type="button"
          onClick={onDismiss}
          className="shrink-0 rounded p-1 text-[#6f7086] transition-colors hover:bg-[#24243a] hover:text-[#e4e4ed]"
          title="Dismiss"
        >
