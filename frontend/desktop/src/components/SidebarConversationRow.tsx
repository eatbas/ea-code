  ...
  return (
    <div className="group/conversation relative">
      <button
        ...
        className={`absolute top-1/2 left-2 z-[1] -translate-y-1/2 rounded p-1 transition-opacity hover:bg-active ${
      ...
      <button
        ...
        className={`relative block w-full rounded-lg py-2 pr-14 pl-9 text-left transition-colors ${
          isActive
            ? "bg-[#252527] text-fg"
      ...
      >
        ...
        <span className={`pointer-events-none absolute top-1/2 right-3 -translate-y-1/2 text-xs text-fg-subtle transition-transform duration-150 ${
          menuOpen ? "-translate-x-5" : ""
        } group-hover/conversation:-translate-x-5`}>
      ...
      <div
        ref={menuRef}
        className="absolute top-2 right-2 z-10 flex items-start"
      >
        <button
          type="button"
          onClick={(event) => {
