...
//! for each running pipeline stage or active conversation turn. Terminal
//! snapshots update the persisted transcript/stages in place, still-running
//! snapshots are handed to a background poll task so we eventually converge,
//! and unreachable Symphony leaves the persisted flag in place so the next
//! startup retries.
...
use crate::conversations::chat::{
    emit_output_delta, emit_status, live_output, maybe_update_provider_session,
...
use crate::models::{
    AgentSelection, ConversationStatus, ConversationStatusEvent, PipelineStageRecord,
    PipelineStageStatusEvent, PipelineState,
};
...
async fn reattach_conversation(app: &AppHandle, workspace_path: &str, conversation_id: &str) {
    ...
    let detail = match persistence::get_conversation(workspace_path, conversation_id) {
        Ok(detail) => detail,
        Err(error) => {
            eprintln!(
            ...
            let _ =
                persistence::forget_persisted_running_conversation(workspace_path, conversation_id);
            return;
        }
    };
    ...
    if running_stages.is_empty() {
        let Some(score_id) = detail.summary.active_score_id.clone() else {
            // No reattachable score remains. Clear the persisted flag so the next
            // foreground read can reconcile this as a stale run.
            let _ =
                persistence::forget_persisted_running_conversation(workspace_path, conversation_id);
            return;
        };

        let message_agent = follow_up_message_agent(pipeline_state.as_ref(), &detail.summary.agent);
        let provider_session_ref = detail.summary.last_provider_session_ref.clone();

        match fetch_score_snapshot(&score_id).await {
            Ok(snapshot) if snapshot.status.is_terminal() => {
                if let Err(error) = apply_terminal_chat(
                    app,
                    workspace_path,
                    conversation_id,
                    message_agent.as_ref(),
                    None,
                    &snapshot,
                ) {
                    eprintln!(
                        "[reattach] {conversation_id}: failed to finalise active score {score_id}: {error}"
                    );
                    return;
                }
                let _ = persistence::forget_persisted_running_conversation(
                    workspace_path,
                    conversation_id,
                );
            }
            Ok(snapshot) => {
                ...
                let session_slot = Arc::new(Mutex::new(provider_session_ref));

                if let Some(delta) = sync_snapshot_output(&live_buffer, &snapshot.accumulated_text)
                {
                    let _ = emit_output_delta(app, conversation_id, &delta);
                }
                let _ = maybe_update_provider_session(
                    app,
                    workspace_path,
                    conversation_id,
                    &session_slot,
            ...
            Err(error) => {
                eprintln!(
                    "[reattach] {conversation_id}: Symphony unreachable for active score {score_id}: {error}"
                );
            }
        }
        return;
...
fn follow_up_message_agent(
    state: Option<&PipelineState>,
    fallback: &AgentSelection,
) -> Option<AgentSelection> {
    state.map(|pipeline_state| {
        latest_coder_agent(&pipeline_state.stages).unwrap_or_else(|| fallback.clone())
    })
}

fn latest_coder_agent(stages: &[PipelineStageRecord]) -> Option<AgentSelection> {
    stages.iter().rev().find_map(|stage| {
        ...
        in_chain
            .then(|| parse_agent_label(&stage.agent_label))
            .flatten()
    })
}
...
fn assistant_text_from_snapshot(
    live_buffer: Option<&Arc<Mutex<String>>>,
    ...
    snapshot
        ...
        .or_else(|| {
            live_buffer.and_then(|buffer| {
                let live = live_output(buffer);
                (!live.trim().is_empty()).then_some(live)
            })
        })
}

fn apply_terminal_chat(
    app: &AppHandle,
    workspace_path: &str,
    conversation_id: &str,
    message_agent: Option<&AgentSelection>,
    live_buffer: Option<&Arc<Mutex<String>>>,
    snapshot: &SymphonyScoreSnapshot,
) -> Result<(), String> {
    let assistant_text = assistant_text_from_snapshot(live_buffer, snapshot);
    let (summary, message) = match message_agent {
        Some(agent) => persistence::finish_turn_with_message_agent(
            workspace_path,
            conversation_id,
...
fn apply_terminal_stage(
    ...
    let Some(updated) =
        update_stage_record(workspace_path, conversation_id, stage_index, |record| {
    ...
    F: FnOnce(&mut PipelineStageRecord),
{
    ...
    if let Err(error) =
        persistence::update_pipeline_stage(workspace_path, conversation_id, &updated)
    {
        eprintln!(
...
fn finalise_summary_from_stages(app: &AppHandle, workspace_path: &str, conversation_id: &str) {
    ...
    match persistence::set_status(workspace_path, conversation_id, status, error) {
        ...
        Err(error) => {
            eprintln!("[reattach] Failed to finalise summary for {conversation_id}: {error}")
        }
    }
}
...
fn spawn_chat_watcher(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let result = poll_until_terminal(&score_id, &stop_flag, |snapshot| {
            if let Some(delta) = sync_snapshot_output(&live_buffer, &snapshot.accumulated_text) {
        ...
        let snapshot = match result {
            ...
            Err(error) => {
                eprintln!("[reattach] Background poll for active score {score_id} failed: {error}");
                return;
            }
        };
        ...
        if let Err(error) = apply_terminal_chat(
            ...
            eprintln!(
                "[reattach] Failed to finalise active score {score_id} for {conversation_id}: {error}"
            );
            return;
        }
...
fn spawn_score_watcher(
    ...
    tauri::async_runtime::spawn(async move {
        ...
        let snapshot = match result {
            Ok(snapshot) => snapshot,
            Err(error) => {
                eprintln!("[reattach] Background poll for score {score_id} failed: {error}");
                return;
            }
        };
