//! Stage execution helpers for iteration.

use crate::models::{PipelineRun, PipelineStage, PipelineStatus, RunEvent, StageEndStatus};
...
pub fn update_run_summary(run_id: &str, session_id: &str, run: &PipelineRun) -> Result<(), String> {
    ...
    let status_str = match run.status {
        PipelineStatus::Completed => Some("completed"),
        PipelineStatus::Failed => Some("failed"),
        PipelineStatus::Cancelled => Some("cancelled"),
        PipelineStatus::Running => Some("running"),
        PipelineStatus::Paused => Some("paused"),
        PipelineStatus::WaitingForInput => Some("waiting_for_input"),
        PipelineStatus::Idle => Some("idle"),
    };

    let verdict_str = run.final_verdict.as_ref().map(|v| match v {
        crate::models::JudgeVerdict::Complete => "COMPLETE",
